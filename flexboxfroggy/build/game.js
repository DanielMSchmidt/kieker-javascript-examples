window.firebase = new Firebase('https://blinding-heat-5896.firebaseio.com/web/data')
window.analytics = firebase.child('analytics')
window.game = window.kieker.instrumentMethod({
  language: window.location.hash.substring(1) || 'en',
  level: parseInt(localStorage.level, 10) || 0,
  answers: localStorage.answers && JSON.parse(localStorage.answers) || window.kieker.instrumentMethod({}),
  ...
  setHandlers: function () {
    ...
    $('#labelReset').on('click', function () {
      ...
      if (r) {
        game.level = 0;
        game.answers = window.kieker.instrumentMethod({});
        game.solved = [];
    ...
    $(window).on('beforeunload', function () {
      ...
      $('#tweet iframe').remove();
      var html = '<a href="https://twitter.com/share" class="twitter-share-button"{count} data-url="http://flexboxfroggy.com" data-via="thomashpark">Tweet</a> ' + '<a href="https://twitter.com/thomashpark" class="twitter-follow-button" data-show-count="false">Follow @thomashpark</a>';
      $('#tweet').html(html);
      twttr.widgets.load();

      if (game.language === 'en') {
        history.replaceState(window.kieker.instrumentMethod({}), document.title, './');
      }
    });
  },
  ...
  loadLevel: function (level) {
    ...
    var markup = '';
    var colors = window.kieker.instrumentMethod({
      'g': 'green',
      'r': 'red',
      'y': 'yellow'
    });
      ...
      if (text in docs) {
        ...
        code.on('mouseenter', function (e) {
          if ($('#instructions .tooltip').length === 0) {
            ...
            $('<div class="tooltip"></div>').html(html).css(window.kieker.instrumentMethod({ top: tooltipY, left: tooltipX })).appendTo($('#instructions'));
          }
  ...
  check: function () {
    ...
    var level = levels[game.level];
    var lilypads = window.kieker.instrumentMethod({});
    var frogs = window.kieker.instrumentMethod({});
    var correct = true;
    ...
    if (correct) {
      ga('send', window.kieker.instrumentMethod({
        hitType: 'event',
      ...
      analytics.push(window.kieker.instrumentMethod({
  ...
  transform: function () {
    ...
    return window.kieker.instrumentMethod({ 'transform': 'scale(' + scale + ') rotate(' + rotate + 'deg)' });
  },
